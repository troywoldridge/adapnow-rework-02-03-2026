...
/**
 * Build/resolve a fast local pricing lookup from Sinalite pricing arrays.
 *
 * Sinalite price rows can vary in shape; we normalize:
 * - option IDs from: productOptions | options | optionIds | combo | combination | response.productOptions
 * - price from: price | price2.price | response.price
 * - packageInfo from: packageInfo | response.packageInfo
 *
 * Key strategy:
 * - IDs are sorted ascending and joined with "-" (stable even if order differs)
 */
...
function keyFromIds(ids: number[]): string {
  // Stable order. (Assumes option IDs uniquely identify the combo)
  return ids.slice().sort((a, b) => a - b).join("-");
}

/**
 * Build a pricing index map from the raw pricing array.
 *
 * If duplicates exist for the same key, the last one wins (matching current behavior).
 */
