...
function siteBaseUrl(): string {
  ...
  return safeText(raw).replace(/\/+$/, "");
}

function emailLogoUrl(): string | undefined {
  const envLogo = safeText(process.env.NEXT_PUBLIC_EMAIL_LOGO_URL);
  if (envLogo) return envLogo;

  // fallback: your Cloudflare logo URL (safe default)
  return "https://imagedelivery.net/pJ0fKvjCAbyoF8aD0BGu8Q/a90ba357-76ea-48ed-1c65-44fff4401600/productTile";
}
...
export async function sendOrderConfirmationEmail(args: {
  ...
  if (Number.isFinite(Number(args.subtotalCents))) {
    lines.push({ label: "Subtotal", value: money(args.subtotalCents || 0, currency) });
  }
  if (Number.isFinite(Number(args.shippingCents))) {
    lines.push({ label: "Shipping", value: money(args.shippingCents || 0, currency) });
  }
  if (Number.isFinite(Number(args.taxCents))) {
    lines.push({ label: "Tax", value: money(args.taxCents || 0, currency) });
  }
  if ((args.discountCents || 0) > 0) {
    lines.push({ label: "Discount", value: `−${money(args.discountCents || 0, currency)}` });
  }
  if ((args.creditsCents || 0) > 0) {
    lines.push({ label: "Loyalty credit", value: `−${money(args.creditsCents || 0, currency)}` });
  }

  const placedAt = safeText(args.placedAt);
  const orderDate = placedAt
    ? new Date(placedAt).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })
    : undefined;
